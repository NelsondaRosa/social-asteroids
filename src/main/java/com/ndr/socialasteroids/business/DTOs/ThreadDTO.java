...
public class ThreadDTO extends RepresentationModel<ThreadDTO>
{
    ...
    public ThreadDTO(ForumThread thread)
    {
        ...
        add(linkTo(methodOn(ForumController.class).getPostsByThread(id.toString())).withRel("posts"));
        add(linkTo(methodOn(UserController.class).getUser(thread.getOwner().getId().toString())).withRel("owenr"));
    }
